...
/*        $('#myModalBuscaRes').on('shown.bs.modal', function (e) {
        ...
        $("#my-file-selector").change(function() {
            ...
            if(archivo.length===0){
                $("#upload-file-info").append("No ha seleccionado archivo para subir");
            }else{
                var nombre = archivo[0].name;
                var peso = returnFileSize(archivo[0].size);
                $("#archivoMemo").append(nombre + " - "+ peso);
            }
        });
        $("#my-filelist-selector").change(function() {
            $("#listaArchivosMemo").html("");
            var archivos = document.getElementById("my-filelist-selector");//Creamos un objeto con el elemento que contiene los archivos: el campo input file, que tiene el id = 'archivos'
            var archivo = archivos.files; //Obtenemos los archivos seleccionados en el imput
            if(archivo.length===0){
                $("#upload-filelist-info").append("No ha seleccionado archivos para subir");
            }else{
